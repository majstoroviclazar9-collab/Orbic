...
const articles = [
  {
    ...
    status: "Published",
    summary:
      "A practical framework for turning dense analytics into clear decisions.",
  ...
  },
  {
    ...
    status: "Published",
    summary:
      "How a small team can move from draft to publish with less friction.",
  ...
  },
  {
    ...
    status: "Published",
    summary:
      "Why time, depth, and returning readers matter more than a single count.",
  ...
  },
  {
    ...
    status: "Published",
    summary:
      "A concise summary of the latest dashboard and publishing improvements.",
  ...
  },
  {
    ...
    status: "Published",
    summary:
      "Keyboard support, focus states, and semantics that improve every workflow.",
  ...
  },
  {
    ...
    status: "Draft",
    summary:
      "The working plan for tutorials, research reports, and community stories.",
  ...
  },
  {
    ...
    status: "Published",
    summary:
      "A simpler way to present trends and recommendations to stakeholders.",
  ...
  },
  {
    ...
    status: "Draft",
    summary:
      "Shared standards for useful, respectful, and accurate contributions.",
...
function isViewName(value) {
  return typeof value === "string" && VIEW_NAMES.includes(value);
}
function isArticleFilter(value) {
  return value === "all" || value === "published" || value === "draft";
}
const authContainer = byId("authContainer");
const dashboardContainer = byId("dashboardContainer");
const loginForm = byId("loginForm");
const registerForm = byId("registerForm");
const loginError = byId("loginError");
const registerError = byId("registerError");
const dynamicContent = byId("dynamicContent");
const navContainer = byId("navContainer");
...
function getUsers() {
  const users = readJson(localStorage, STORAGE_USERS, null);
...
function getLoggedUser() {
  const user = readJson(sessionStorage, SESSION_USER, null);
...
function getPreferences() {
  const stored = readJson(localStorage, PREFS_KEY, {});
  return {
    emailNotifications:
      isObject(stored) && typeof stored.emailNotifications === "boolean"
        ? stored.emailNotifications
        : true,
    compactNavigation:
      isObject(stored) && typeof stored.compactNavigation === "boolean"
        ? stored.compactNavigation
        : false,
  };
}
...
function setTheme(theme, announce = false) {
  ...
  document.querySelectorAll("[data-theme-option]").forEach((button) => {
    button.classList.toggle(
      "selected",
      button.dataset.themeOption === nextTheme,
    );
    button.setAttribute(
      "aria-pressed",
...
function updateUserHeader() {
  ...
  byId("userGreeting").textContent = `Hi, ${firstName}`;
  byId("profileAvatar").textContent = firstName.charAt(0).toUpperCase();
}
...
function openDashboard() {
  ...
  updateUserHeader();
  applyPreferences();
  byId("menuButton").setAttribute(
    "aria-expanded",
    String(
      window.innerWidth > 900 && !navContainer.classList.contains("collapsed"),
    ),
  );
...
function getFilteredArticles() {
  ...
  return articles.filter((article) => {
    ...
    const matchesStatus =
      articleStatus === "all" || article.status.toLowerCase() === articleStatus;
...
function loadView(view, focusContent = true) {
  const renderers = {
  ...
  const safeView = isViewName(view) ? view : "dashboard";
  currentView = safeView;
  profileEditing = safeView === "profile" ? profileEditing : false;
  dynamicContent.innerHTML = renderers[safeView]();
  document.querySelectorAll(".nav-option[data-view]").forEach((button) => {
...
function openArticle(articleId) {
  ...
  openModal(
    `<span class="status-pill ${article.status === "Draft" ? "draft" : ""}">${article.status}</span><h2 id="modalTitle" style="margin-top:14px">${escapeHtml(article.title)}</h2><p class="article-meta">${formatDate(article.date)} · ${formatNumber(article.views)} views · ${article.comments} comments</p><p class="modal-copy">${escapeHtml(article.body)}</p>`,
  );
}
...
function submitGlobalSearch(value) {
  ...
  byId("desktopSearch").value = articleQuery;
  byId("mobileSearch").value = articleQuery;
  articleStatus = "all";
  loadView("articles");
}
function exportReport() {
  const rows = [
    "Month,Views (thousands)",
    "January,42",
    "February,55",
    "March,49",
    "April,68",
...
registerForm.addEventListener("submit", (event) => {
  ...
  const confirmation = byId("regConfirmPassword").value;
  registerError.classList.remove("success");
  if (name.length < 2) {
...
byId("showRegisterLink").addEventListener("click", showRegister);
byId("showLoginLink").addEventListener("click", () => showLogin());
...
byId("menuButton").addEventListener("click", toggleNavigation);
navOverlay.addEventListener("click", closeNavigation);
byId("logoutButton").addEventListener("click", logout);
byId("profileButton").addEventListener("click", () => loadView("profile"));
...
byId("markReadButton").addEventListener("click", () => {
  byId("notificationDot").classList.add("hidden");
  closeNotifications();
...
dynamicContent.addEventListener("click", (event) => {
  ...
  const articleButton = target.closest("[data-read-article]");
  if (articleButton) {
  ...
  if (target.closest("#openPasswordForm")) {
    byId("passwordForm").classList.remove("hidden");
...
dynamicContent.addEventListener("input", (event) => {
  ...
  const search = byId("articleSearch");
  search.focus();
...
dynamicContent.addEventListener("change", (event) => {
  ...
  if (target.id === "compactNavigation") {
    savePreferences({
      compactNavigation: target.checked,
    });
...
dynamicContent.addEventListener("submit", (event) => {
  ...
  if (event.target.id === "profileEditForm") {
    ...
    const emailAlreadyUsed = users.some(
      (user) =>
        ...
        user.email.toLowerCase() !== current.email.toLowerCase(),
    );
    if (emailAlreadyUsed) {
  ...
  if (event.target.id === "passwordForm") {
    ...
    const users = getUsers();
    const userIndex = users.findIndex(
      (user) => user.email.toLowerCase() === logged.email.toLowerCase(),
    );
